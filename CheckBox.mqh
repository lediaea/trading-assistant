...
//| Resources                                                        |
//+------------------------------------------------------------------+
#resource "Assest\\check-on_dark.bmp"
#resource "Assest\\check-on_white.bmp"
#resource "Assest\\check-off.bmp"
//+------------------------------------------------------------------+
//| Class CCheckBox                                                  |
...
public:
   ...
   bool              FontSize(const color value) { return(m_label.FontSize(value));}
   //--- state
...
//--- create
   if(!m_button.Create(m_chart_id,m_name+"Button",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_button.BmpNames("::Assest\\check-off.bmp",DARK_MODE ? "::Assest\\check-on_dark.bmp" : "::Assest\\check-on_white.bmp"))
      return(false);
   if(!Add(m_button))
      return(false);
